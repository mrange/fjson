...
let rec randomizer max_level current_level max_members (r: Random) (wsg : int->Random->Whitespace) (sg : Random->WSString)=
    ...
    match x with 
        ...
        | 6 ->  
                ...
                let os = [| for i in 1..oc -> ((sg r) , randomizer max_level (current_level + 1) max_members r wsg sg) |]
                ObjectValue (os, (wsg current_level r))
        | _ ->  ArrayValue ([||], (wsg current_level r))

let indenter current_level random = ("", "")

let string_generator random = ("test", ("", ""))
